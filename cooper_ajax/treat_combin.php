<?php
	...
	//處置
	$conn->exec("truncate table treat_record_tmp");
	$sql = "SELECT * FROM op_opasm where `健保代號2`!=' ' ";
	$result=$db->query($sql);
	while ($row = $result->fetch( PDO::FETCH_ASSOC)) {
		...
		if ($trcode2!=''){
			$pamt=$price1*$qty1;
			$sql="insert into treat_record_tmp(regsn,uploadD,sickn,fdi,trcode,sickno,side,treat_memo,nums,punitfee,add_percent,pamt,icd10,icd10pcs1,icd10pcs2,cc) values
